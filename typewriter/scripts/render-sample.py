#!/usr/bin/env python3
"""Render 荷塘月色 mixed CJK/EN coding sample for CourierPrimeZhuque."""
...
// coding sample · 中英混排对齐检查 · typewriter mono
const moon = "荷塘月色"; // full moon over the lotus pond
function walk(path: string): void {
  // day by day 走过的 path，观察 leaf 与 flower 的 rhythm
  console.log(`path=${path}, width=600/1200`);
}
// 两英文字母应严格对齐一汉字：aa中bb文cc混dd排
// |a|b|中|c|d|文|  →  中文 cell=1200, 英文 cell=600
aa中bb文cc混dd排ee齐ff检
|a|b|中|c|d|文|e|f|混|
// 0 O o 1 l I |  (distinguishability smoke)
const id = 0x1lI|O; // not real code — glyph check
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--font",
        type=Path,
        default=root / "out" / "nerd" / "CourierPrimeZhuqueNFM-Regular.ttf",
    )
    ap.add_argument("--out-dir", type=Path, default=root / "samples" / "rendered")
    ap.add_argument("--title", default="CourierPrimeZhuque NFM · EN 600 / CJK 1200")
