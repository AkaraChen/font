#!/usr/bin/env bash
# Shared helpers for typewriter/ build scripts.
set -euo pipefail

TYPEWRITER_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "${TYPEWRITER_ROOT}/.." && pwd)"
# shellcheck disable=SC1091
source "${TYPEWRITER_ROOT}/pins.env"

WORK_DIR="${TYPEWRITER_ROOT}/work"
DOWNLOADS_DIR="${WORK_DIR}/downloads"
EXTRACT_DIR="${WORK_DIR}/src"
VENV_DIR="${WORK_DIR}/venv"
OUT_DIR="${TYPEWRITER_ROOT}/out"
