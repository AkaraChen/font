#!/usr/bin/env python3
"""Render mixed CJK/EN sample for geometric dual-width experiments."""
...
// 易混字形 · geometric: o O 0 l I 1 | 圆方构成感
const id = 0OolI1|; // o O 0 · l I 1 · pipe
// 两英文字母应严格对齐一汉字：aa中bb文cc混dd排
// |a|b|中|c|d|文|  →  中文 cell=1100, 英文 cell=550
aa中bb文cc混dd排ee齐ff检
|a|b|中|c|d|文|e|f|混|
// ligatures (if font has calt): != => <= >= == === -> <-
if (x != y && a => b || c <= d) { return foo === bar; }"""
...
def render(
    ...
    out = out_dir / f"{prefix}-{theme}.png"
    img.save(out, "PNG")
...
def main() -> int:
    ...
    ap.add_argument(
        "--font",
        type=Path,
        default=root / "out" / "ExpGeometricDual-Regular.ttf",
    )
    ap.add_argument("--out-dir", type=Path, default=root / "samples" / "rendered")
    ap.add_argument("--title", default="Geometric merge · EN 550 / CJK 1100")
    ap.add_argument("--body-file", type=Path, default=None)
    ap.add_argument("--size", type=int, default=15)
    ap.add_argument("--prefix", default="sample", help="output filename prefix")
    ap.add_argument("--themes", default="dark", help="comma list: dark,light")
    args = ap.parse_args()
