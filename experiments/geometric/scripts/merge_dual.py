#!/usr/bin/env python3
"""Experimental dual-width merge: Latin mono + CJK sans (Regular-only ok).

Default product metrics: EN 550 / CJK 1100 (strict 2:1).

Character policy (same as sans/scripts/merge_plex.py):
- Latin mono (X-scaled to EN_ADV): Latin, digits, programming symbols; GSUB/GPOS kept.
- CJK SC (advance expanded & centred to CJK_ADV): CJK ranges + missing codepoints.
"""
...
def scale_glyph_x(glyph, glyf_table, scale: float) -> None:
    ...
    if glyph.isComposite():
        for component in glyph.components:
            ...
            # fontTools GlyphComponent.transform is either absent, a 2x2 matrix
            # [[xx, xy], [yx, yy]], or occasionally a flat 4-tuple.
            t = getattr(component, "transform", None)
            if t is not None:
                if (
                    ...
                    # simple (xScale, yScale)
                    component.transform = (float(t[0]) * scale, float(t[1]))
...
def rename_family(
    ...
    add(
        5,
        f"1.000;KIT-288;{family} experimental merge (EN {en_adv} / CJK {cjk_adv})",
    )
...
def detect_mono_adv(font_path: Path) -> int:
    f = TTFont(font_path)
    cm = f.getBestCmap() or {}
    for ch in "aAmi0":
        g = cm.get(ord(ch))
        if g:
            ...
            return adv
    f.close()
    raise SystemExit(f"cannot detect mono advance in {font_path}")


def main() -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--mono-regular", "--latin-regular", dest="latin_regular", type=Path, required=True)
    p.add_argument("--mono-bold", "--latin-bold", dest="latin_bold", type=Path, default=None)
    p.add_argument("--sc-regular", type=Path, required=True)
    p.add_argument("--sc-bold", type=Path, default=None)
    p.add_argument("--out-dir", type=Path, required=True)
    p.add_argument("--en-adv", type=int, default=550)
    p.add_argument("--cjk-adv", type=int, default=1100)
    p.add_argument("--mono-src-adv", "--latin-src-adv", dest="latin_src_adv", type=int, default=0)
    p.add_argument("--family", default="ExpGeometric Dual")
    p.add_argument("--family-ps", default="ExpGeometricDual")
    ...
    latin_src_adv = args.latin_src_adv or detect_mono_adv(args.latin_regular)
    print(f"latin_src_adv={latin_src_adv}")
    ...
    if args.latin_bold and args.sc_bold:
        pairs.append(
            (
                args.latin_bold,
                args.sc_bold,
    ...
    elif args.latin_bold or args.sc_bold:
        print("note: bold requires both --mono-bold and --sc-bold; skipping bold", file=sys.stderr)
