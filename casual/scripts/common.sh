...
ensure_python() {
  ...
  if [[ -n "${FONTKIT_PYTHON:-}" ]]; then
    "${FONTKIT_PYTHON}" -c "import fontTools, pathops, PIL, freetype, numpy, uharfbuzz" \
      || die "FONTKIT_PYTHON=${FONTKIT_PYTHON} cannot import: fontTools, pathops, PIL, freetype, numpy, uharfbuzz"
    return 0
  fi
