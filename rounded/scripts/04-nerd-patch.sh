...
mapfile -t BASE_FONTS < <(
  find "${OUT_DIR}" -maxdepth 1 -type f \( -name 'IosevkaCurlyRHRDual-*.ttf' -o -name 'IosevkaCurlyRHRDual-*.otf' \) | sort
)
...
RENAME="${ROUNDED_ROOT}/scripts/rename_nerd_family.py"
...
FIX_NERD="${ROUNDED_ROOT}/scripts/fix-nerd-widths.py"
NARROW="${ROUNDED_ROOT}/scripts/narrow-symbol-widths.py"
FIX_METRICS="${ROUNDED_ROOT}/scripts/fix-terminal-metrics.py"
