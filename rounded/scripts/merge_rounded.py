...
Character policy
----------------
- Iosevka Curly (mono cell → EN_ADV): Latin, digits, programming symbols,
  half-width punctuation. GSUB/GPOS/GDEF kept (ligatures / ss / cv when present).
...
def is_cjk_side(cp: int) -> bool:
    """True for CJK-side codepoints imported from RHR (not Latin)."""
    ranges = (
...
def rename_family(
    ...
    add(
        5,
        f"1.000;KIT;{family} merge (Iosevka Curly + Resource Han Rounded; "
...
# Unicode ranges kept from Iosevka for coding (drop bulk cv/ss glyph soup).
_LATIN_UNICODES_RANGES = (
...
def subset_latin_coding(font: TTFont) -> None:
    """Shrink Iosevka so Latin+RHR fits TrueType numGlyphs ≤ 65535.

    Keep coding unicodes + default ligature/layout features only (not every
    ss/cv alternate), otherwise Curly alone is ~46k glyphs.
    """
    ...
    for a, b in _LATIN_UNICODES_RANGES:
        for cp in range(a, b + 1):
            if cp in cm:
                unicodes.add(cp)

    options = Options()
    # Keep programming ligatures / composition; drop bulk stylistic alternates
    # so glyph count stays manageable after CJK import (TrueType ≤65535).
    ...
    print(
        f"  Subset Latin coding unicodes={len(unicodes)} "
        f"glyphs {before} → {after}"
    )
...
def prepare_latin(
    ...
    *,
    ...
    # Must subset before CJK import — full Iosevka Curly ≈ 46k glyphs.
    subset_latin_coding(latin)
    ...
    if target_upm and current_upm != target_upm:
        ...
        # After UPM normalize, mono cell becomes latin_src_adv
...
def merge_pair(
    ...
    *,
    ...
    print("  Importing CJK-side ranges from Resource Han Rounded")
...
def main() -> int:
    ...
    p.add_argument("--en-adv", type=int, default=500)
    p.add_argument("--cjk-adv", type=int, default=1000)
