...
sans/
  ...
  scripts/
    ...
    03-nerd-patch.sh       # Nerd complete + rename + metric hygiene
    04-verify.sh
    merge_plex.py
...
./scripts/03-nerd-patch.sh      # Nerd complete + half-cell icons + mono flags
./scripts/04-verify.sh          # hard-fail if advances / flags / features drift
```
...
5. Systematic EAW / Powerline / math symbol visual QA (beyond Nerd half-cell gate)

## Verify

```bash
python3 scripts/verify-2to1.py --expect-half 550 --check-nerd out/nerd/LilexSansSCNFM-*.ttf
