...
#
# Strategy mirrors pixel/scripts/04-nerd-patch.sh and serif/scripts/05-nerd-patch.sh:
...
#   - rename → short Windows-safe NFM family
#   - fix Nerd PUA advances + terminal mono metrics (FontForge clears isFixedPitch)
set -euo pipefail
...
# Intermediate Dual fonts only (not already-patched Nerd products)
mapfile -t BASE_FONTS < <(
  find "${OUT_DIR}" -maxdepth 1 -type f \( -name 'LilexSansSCDual-*.ttf' -o -name 'LilexSansSCDual-*.otf' \) | sort
)
...
if [[ ${#BASE_FONTS[@]} -eq 0 ]]; then
  die "no fonts in ${OUT_DIR}; run 02-merge.sh first"
fi
